    ...
    fetch("/asr", { method: "post", body: data })
        ...
        .then((response) => {
            ...
            for (i of response["r"]) {
                ...
                response_se_audio_url = i["response_se_audio_url"];
                response = i["response"];
                response_raw_2d_url = i['response_raw_2d_url'];
                response_raw_3d_url = i['response_raw_3d_url'];
                response_se_2d_url = i['response_se_2d_url'];
                response_se_3d_url = i['response_se_3d_url'];
                ...
                if (response_code === 0) {
                    response.forEach(function (model_ans) {
                        RECOGNITION_RESULT.insertAdjacentHTML(
                            ...
                            TR_PATTERN.format(
                                response_audio_url,
                                response_se_audio_url,
                                model_ans["time"],
                                model_ans["without_punc_text"],
                                model_ans['text'],
                                model_ans['itn_text'],
                                response_raw_2d_url, response_raw_3d_url, response_se_2d_url, response_se_3d_url
                            )
                        );
                    });
                } else {
